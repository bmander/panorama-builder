...
// *unlock* each axis. Photo-owned axes use the per-photo count; station-owned
...
// photo and per station. Every Observation in problem.Observations already
// carries a ControlPointID (the solver only ingests matched rows), so no
// filter is needed here.
func matchedObsCounts(p Problem) (perPhoto, perStation map[string]int) {
...
func stationAutoLock(count int) StationLocks {
	return StationLocks{
...
// observations.
func photoAutoLock(count int) PhotoLocks {
	return PhotoLocks{
