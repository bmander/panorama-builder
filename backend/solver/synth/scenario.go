...
// using solver.BearingFromStationToCP — the same WGS84 + refraction bearing
...
// CPs through the World's photos using solver.BearingFromStationToCP — the
...
func Build(w World) (solver.Problem, error) {
	...
	for cpIdx, photoIdxs := range w.VisibleIn {
		...
		for _, pIdx := range photoIdxs {
			...
			if !ok {
				return solver.Problem{}, fmt.Errorf("synth.Build: photo[%d] references unknown station %q", pIdx, p.StationID)
			}
