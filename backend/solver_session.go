...
func (s *Server) writebackChangesInSession(ctx context.Context, sessionID string, changes []wire.EntityChange) error {
	...
	// Push a pre-solve checkpoint onto the undo stack so the whole writeback
	// (one user action) undoes in one step. The solve request sends no
	// X-Action-Id, so requireSession recorded nothing; the writeback owns its
	// checkpoint, and only when it actually changes the journal (len > 0,
	// guaranteed by the early return above).
	snap, err := captureJournalSnapshot(ctx, tx, sessionID)
	if err != nil {
		return fmt.Errorf("capture journal snapshot: %w", err)
	}
	if err := pushCheckpoint(ctx, tx, sessionID, stackUndo, "solve", snap); err != nil {
		return err
	}
