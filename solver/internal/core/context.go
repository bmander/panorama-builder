...
// composeChanges() to emit the wire.Result.Changes diff. Anything that touches
...
type solveContext struct {
	...
	cpBasis      []geom.ENUBasis
	cpOffset     [][3]float64

	obs      []wire.Observation // observations in scope for this mode
...
// parameter slots.
func buildContext(problem wire.Problem, cfg Config) (*solveContext, error) {
	...
	problem.Stations = append([]wire.Station(nil), problem.Stations...)
	problem.Photos = append([]wire.Photo(nil), problem.Photos...)
	...
	if cfg.Mode == wire.ModeJoint && fullyLockedCount < 2 {
		return nil, wire.ErrUnderconstrainedGauge
	}

	if cfg.Mode == wire.ModeSingleStation {
		if _, ok := c.stationIdx[cfg.FocusID]; !ok {
			return nil, wire.ErrFocusNotFound
		}
	}

	c.stationOriginLLA = make([][3]float64, len(problem.Stations))
	c.stationOriginECEF = make([][3]float64, len(problem.Stations))
	c.stationBasis = make([]geom.ENUBasis, len(problem.Stations))
	c.stationOffset = make([][3]float64, len(problem.Stations))
	for i, s := range problem.Stations {
		c.stationOriginLLA[i] = [3]float64{s.Lat, s.Lng, s.Alt}
		x, y, z := geom.LLAToECEF(s.Lat, s.Lng, s.Alt)
		c.stationOriginECEF[i] = [3]float64{x, y, z}
		c.stationBasis[i] = geom.LocalENUBasis(s.Lat, s.Lng)
	}

	c.photoPose = make([]wire.Pose, len(problem.Photos))
	for i, p := range problem.Photos {
	...
	c.cpBasis = make([]geom.ENUBasis, len(problem.ControlPoints))
	for i, lla := range c.cpOriginLLA {
		x, y, z := geom.LLAToECEF(lla[0], lla[1], lla[2])
		c.cpOriginECEF[i] = [3]float64{x, y, z}
		c.cpBasis[i] = geom.LocalENUBasis(lla[0], lla[1])
	}
...
func pickGaugeStation(stations []wire.Station) (*wire.Station, int) {
...
// mode. wire.ModeSingleStation locks all stations and CPs; wire.ModeSingleControlPoint
// locks everything except the focus CP.
func (c *solveContext) scopeAndSlots() error {
	switch c.cfg.Mode {
	case wire.ModeJoint:
		c.obs = append([]wire.Observation(nil), c.problem.Observations...)
	...
	case wire.ModeSingleControlPoint:
...
func (c *solveContext) appendCPSlot(cp wire.ControlPoint, axis int, name string) {
...
func (c *solveContext) appendPhotoSlots(photos []wire.Photo) {
...
func currentECEF(origin [3]float64, basis geom.ENUBasis, off [3]float64) [3]float64 {
	if off == ([3]float64{}) {
		return origin
	}
	x, y, z := geom.ApplyOffsetECEF(origin, basis, off[axisEast], off[axisNorth], off[axisUp])
...
func (c *solveContext) scopedStationLocks(s wire.Station) (east, north, up bool) {
	switch c.cfg.Mode {
	case wire.ModeJoint:
		return s.Locks.Lng, s.Locks.Lat, s.Locks.Alt
	case wire.ModeSingleStation, wire.ModeSingleControlPoint:
...
func (c *solveContext) scopedPhotoLocks(p wire.Photo) wire.PhotoLocks {
	all := wire.PhotoLocks{
...
func (c *solveContext) scopedCPFullyLocked(cp wire.ControlPoint) bool {
	switch c.cfg.Mode {
	case wire.ModeJoint:
		return false
	case wire.ModeSingleControlPoint:
...
// composeChanges emits one wire.EntityChange per entity whose effective LLA (or
// photo intrinsic) moved by more than the FD scale. wire.Photo slots use the raw
// initial/final slot values; station and CP positions are reconstructed
// from origin + slot-offset and round-tripped through geom.ECEFToLLA so the
...
// member's slot).
func (c *solveContext) composeChanges(initial, final []float64) []wire.EntityChange {
	type acc struct {
	...
	// wire.Photo slots: scalar before/after, no LLA round-trip.
	...
	for i, st := range c.problem.Stations {
		...
		afterLat, afterLng, afterAlt := geom.ECEFToLLA(ecef[0], ecef[1], ecef[2])
	...
	for i := range c.problem.ControlPoints {
		...
		lat, lng, alt := geom.ECEFToLLA(ecef[0], ecef[1], ecef[2])
	...
	cpAxisAfter := func(cp wire.ControlPoint, axis int) float64 {
	...
	var out []wire.EntityChange
	for id, a := range stations {
		out = append(out, wire.EntityChange{Kind: "station", ID: id, Before: a.before, After: a.after})
	}
	for id, a := range photos {
		out = append(out, wire.EntityChange{Kind: "photo", ID: id, Before: a.before, After: a.after})
	}
	for id, a := range cps {
		out = append(out, wire.EntityChange{Kind: "control_point", ID: id, Before: a.before, After: a.after})
	}
	return out
}
...
func emitLLADiff(
	...
	if math.Abs(afterLLA[0]-beforeLLA[0])*geom.MPerDegLat > fdEpsPosM {
		before[prefix+"lat"] = beforeLLA[0]
		after[prefix+"lat"] = afterLLA[0]
	}
	cosLat := math.Cos(beforeLLA[0] * math.Pi / 180)
	if math.Abs(afterLLA[1]-beforeLLA[1])*geom.MPerDegLat*cosLat > fdEpsPosM {
