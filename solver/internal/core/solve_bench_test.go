...
func buildJointBenchWorld(stationCount, cpCount int, seed int64) synth.World {
	...
	for i := 0; i < stationCount; i++ {
		...
		if i < 2 {
			s.Locks = wire.StationLocks{Lat: true, Lng: true, Alt: true}
		}
		stations[i] = s
	}
...
func perturbForBench(p *wire.Problem, seed int64) {
...
func BenchmarkSolveJoint15x50(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		...
		_, err := core.Solve(p, core.Config{Mode: wire.ModeJoint, MaxIters: 50})
		if err != nil {
			b.Fatal(err)
		}
	}
...
func BenchmarkSolveJoint15x50_MaxIters200(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		p := cloneProblem(probTemplate)
		perturbForBench(&p, int64(i))
		_, err := core.Solve(p, core.Config{Mode: wire.ModeJoint, MaxIters: 200})
		if err != nil {
			b.Fatal(err)
		}
	}
...
func cloneProblem(p wire.Problem) wire.Problem {
	out := wire.Problem{
		Stations:      append([]wire.Station(nil), p.Stations...),
		Photos:        append([]wire.Photo(nil), p.Photos...),
		ControlPoints: append([]wire.ControlPoint(nil), p.ControlPoints...),
		Observations:  append([]wire.Observation(nil), p.Observations...),
	}
	return out
}
