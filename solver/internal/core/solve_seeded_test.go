...
func stationCentroid(stations []wire.Station) (lat, lng float64) {
...
// findCPChange returns the wire.EntityChange for the given CP id, or nil.
func findCPChange(changes []wire.EntityChange, id string) *wire.EntityChange {
	for i := range changes {
		c := &changes[i]
...
func TestSolveJointWithSeedRecoversNullCP(t *testing.T) {
	...
	cpTrueLat, cpTrueLng, cpTrueAlt := geom.ENUToLatLngAlt(75, 250, 5, gaugeLat, gaugeLng, 0)
	w := fourStationWorld(
	...
	res, err := core.SolveJointWithSeed(prob, []string{"cp1"}, core.Config{MaxIters: 80})
	if err != nil {
...
func TestSolveJointWithSeedDoesNotPerturbWellLocatedNeighbors(t *testing.T) {
	...
	cp1Lat, cp1Lng, cp1Alt := geom.ENUToLatLngAlt(75, 250, 5, gaugeLat, gaugeLng, 0)
	cp2Lat, cp2Lng, cp2Alt := geom.ENUToLatLngAlt(40, 240, 0, gaugeLat, gaugeLng, 0)
	cp3Lat, cp3Lng, cp3Alt := geom.ENUToLatLngAlt(-30, 260, 0, gaugeLat, gaugeLng, 0)
	w := fourStationWorld(
		[]synth.TrueCP{
...
func TestSolveJointWithSeedEmptySeedListEqualsPlainSolve(t *testing.T) {
	...
	// over Solve(wire.Mode=wire.ModeJoint). Build the same scenario as
	...
	// wire.Photo intrinsics need photo_az free for this regression to mirror the
	// existing bundle-adjustment fixture; rebuild photos with photo_az free.
	for i := range w.Photos {
		w.Photos[i].Locks = wire.PhotoLocks{PhotoTilt: true, PhotoRoll: true, SizeRad: true}
	}
	prob, err := synth.Build(w)
	if err != nil {
		t.Fatal(err)
	...
	probA := prob
	probA.Stations = append([]wire.Station(nil), prob.Stations...)
	probA.Photos = append([]wire.Photo(nil), prob.Photos...)
	probA.ControlPoints = append([]wire.ControlPoint(nil), prob.ControlPoints...)
	probA.Observations = append([]wire.Observation(nil), prob.Observations...)
	perturb(&probA)

	probB := prob
	probB.Stations = append([]wire.Station(nil), prob.Stations...)
	probB.Photos = append([]wire.Photo(nil), prob.Photos...)
	probB.ControlPoints = append([]wire.ControlPoint(nil), prob.ControlPoints...)
	probB.Observations = append([]wire.Observation(nil), prob.Observations...)
	perturb(&probB)

	resPlain, err := core.Solve(probA, core.Config{Mode: wire.ModeJoint, MaxIters: 80})
	if err != nil {
		t.Fatalf("plain solve: %v", err)
	}
	resWrapped, err := core.SolveJointWithSeed(probB, nil, core.Config{MaxIters: 80})
	if err != nil {
...
func TestSolveJointWithSeedSkipsUnknownCPID(t *testing.T) {
	...
	res, err := core.SolveJointWithSeed(prob, []string{"does-not-exist"}, core.Config{MaxIters: 80})
	if err != nil {
		t.Fatalf("solve: %v", err)
	}
	if res.Diverged {
...
func TestSolveJointWithSeedTrivialJointAfterPreSolve(t *testing.T) {
	...
	// phase then has nothing to do (already at residual tolerance). wire.Result
