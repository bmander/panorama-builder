...
import (
	...
	"github.com/bmander/panorama-builder/solver/internal/core/synth"
)
...
func fullyLocked(id string, lat, lng float64) synth.TrueStation {
	return synth.TrueStation{
		...
		Lat:   lat,
		Lng:   lng,
		Locks: wire.StationLocks{Lat: true, Lng: true},
	}
}
...
func basicPose(az float64) wire.Pose {
	return wire.Pose{
		PhotoAz:   az,
		PhotoTilt: 0,
		PhotoRoll: 0,
...
func offsetLatLng(east, north float64) (float64, float64) {
	lat, lng, _ := geom.ENUToLatLngAlt(east, north, 0, gaugeLat, gaugeLng, 0)
	return lat, lng
}
...
func TestSolveIdentityNoChange(t *testing.T) {
	...
	res, err := core.Solve(prob, core.Config{Mode: wire.ModeJoint})
	if err != nil {
...
func TestSolveSingleStationRecoversPhotoAz(t *testing.T) {
	...
	w := synth.World{
		...
		Photos: []synth.TruePhoto{
			...
			{ID: "p1", StationID: "st1", Pose: basicPose(truePhotoAz),
				Locks: wire.PhotoLocks{PhotoTilt: true, PhotoRoll: true, SizeRad: true}},
		},
		ControlPoints: []synth.TrueCP{
			{ID: "cp1", EstLat: cpLat, EstLng: cpLng, EstAlt: 0,
				Locks: wire.CPLocks{EstLat: true, EstLng: true, EstAlt: true}},
		},
		VisibleIn: [][]int{{0}},
	}
	prob, err := synth.Build(w)
	...
	res, err := core.Solve(prob, core.Config{Mode: wire.ModeSingleStation, FocusID: "st1"})
	if err != nil {
		t.Fatalf("solve: %v", err)
	}
	if res.Diverged {
	...
	if math.Abs(geom.WrapPi(got-truePhotoAz)) > 1e-4 {
...
func TestSolveSingleStationJointPoseRecovery(t *testing.T) {
	...
	for _, c := range res.Changes {
		...
		if math.Abs(geom.WrapPi(az-exp)) > 1e-3 {
...
func TestSolveLockHonored(t *testing.T) {
	...
	w := synth.World{
		...
		Photos: []synth.TruePhoto{
			{ID: "p1", StationID: "st1", Pose: basicPose(0),
				Locks: wire.PhotoLocks{PhotoAz: true, PhotoTilt: true, PhotoRoll: true, SizeRad: true}},
		},
		ControlPoints: []synth.TrueCP{
...
func TestSolveControlPointRecovery(t *testing.T) {
	...
	res, err := core.Solve(prob, core.Config{Mode: wire.ModeSingleControlPoint, FocusID: "cp1"})
	if err != nil {
		t.Fatalf("solve: %v", err)
	}
	if res.Diverged {
...
func TestSolveJointGaugeRequired(t *testing.T) {
	...
	lockAll := wire.CPLocks{EstLat: true, EstLng: true, EstAlt: true}
	w := synth.World{
		Stations: []synth.TrueStation{
	...
	_, err = core.Solve(prob, core.Config{Mode: wire.ModeJoint})
	if err != wire.ErrUnderconstrainedGauge {
		t.Errorf("want wire.ErrUnderconstrainedGauge; got %v", err)
	}
}
...
func TestSolveJointBundleAdjustment(t *testing.T) {
	...
	// are fully locked (gauge); station 2 and 3 have lat/lng free. wire.Photo
	...
	for i, p := range stPositions {
		...
		if i < 2 {
			st.Locks = wire.StationLocks{Lat: true, Lng: true}
		}
		stations[i] = st
	}
	...
	for i := range stPositions {
		photos[i] = synth.TruePhoto{
			...
			Pose:  wire.Pose{PhotoAz: 0, PhotoTilt: 0, PhotoRoll: 0, SizeRad: 2 * math.Pi / 3, Aspect: 1.5},
			Locks: wire.PhotoLocks{PhotoTilt: true, PhotoRoll: true, SizeRad: true},
		}
	}
	...
	for i, p := range cpPositions {
		lat, lng, alt := geom.ENUToLatLngAlt(p[0], p[1], p[2], gaugeLat, gaugeLng, 0)
	...
	res, err := core.Solve(prob, core.Config{Mode: wire.ModeJoint, MaxIters: 80})
	if err != nil {
		t.Fatalf("solve: %v", err)
	}
	if res.Diverged {
...
func TestSolveRecoversK1K2(t *testing.T) {
	...
	// non-zero K1, K2 (Brown-Conrady radial distortion). wire.Pose, station, and
	...
	res, err := core.Solve(prob, core.Config{
		Mode: wire.ModeSingleStation, FocusID: "st1",
		KRegLambda: -1,
	})
	if err != nil {
		t.Fatalf("solve: %v", err)
...
func threeStationCPPair(t *testing.T, cpAENU, cpBENU [3]float64) wire.Problem {
	t.Helper()
	cpALat, cpALng, cpAAlt := geom.ENUToLatLngAlt(cpAENU[0], cpAENU[1], cpAENU[2], gaugeLat, gaugeLng, 0)
	cpBLat, cpBLng, cpBAlt := geom.ENUToLatLngAlt(cpBENU[0], cpBENU[1], cpBENU[2], gaugeLat, gaugeLng, 0)
	allPhotoLocks := wire.PhotoLocks{PhotoAz: true, PhotoTilt: true, PhotoRoll: true, SizeRad: true}
	allStationLocks := wire.StationLocks{Lat: true, Lng: true, Alt: true}
...
func cpByID(prob wire.Problem, id string) *wire.ControlPoint {
	for i := range prob.ControlPoints {
...
func cpChangeAfter(res wire.Result, id string) map[string]float64 {
	for _, c := range res.Changes {
...
func TestSolveCPConstraintPlumbSharesLatLng(t *testing.T) {
	...
	prob.CPConstraints = []wire.CPConstraint{
