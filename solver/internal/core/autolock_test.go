...
func TestApplyAutoLocksZeroObs(t *testing.T) {
	p := wire.Problem{
		Stations: []wire.Station{{ID: "s1"}},
		Photos:   []wire.Photo{{ID: "p1", StationID: "s1"}},
	}
	applyAutoLocks(&p)
	got := p.Photos[0].Locks
	want := wire.PhotoLocks{PhotoAz: true, PhotoTilt: true, PhotoRoll: true, SizeRad: true, K1: true, K2: true}
	if got != want {
...
func TestSolveWithOneObsDoesNotMoveRoll(t *testing.T) {
	...
	cpLat, cpLng, cpAlt := geom.ENUToLatLngAlt(0, 50, 0, gLat, gLng, 0)
	// One observation, well inside the FOV (90° H × ~67° V).
	prob := wire.Problem{
		Stations: []wire.Station{
			{ID: "s1", Lat: gLat, Lng: gLng, Alt: 0,
				Locks: wire.StationLocks{Lat: true, Lng: true, Alt: true}},
			{ID: "s2", Lat: gLat, Lng: gLng + 0.0001, Alt: 0,
				Locks: wire.StationLocks{Lat: true, Lng: true, Alt: true}},
		},
		Photos: []wire.Photo{
			{ID: "p1", StationID: "s1", Pose: wire.Pose{
	...
	res, err := Solve(prob, Config{Mode: wire.ModeJoint})
	if err != nil {
...
// wire.ModeSingleControlPoint (which fully locks photos) and then a wire.ModeJoint
...
func TestApplyAutoLocksHonorsManualLock(t *testing.T) {
	p := wire.Problem{
		...
		Photos: []wire.Photo{{
			ID: "p1", StationID: "s1",
			Locks: wire.PhotoLocks{PhotoAz: true}, // user manually locked
		}},
		Observations: []wire.Observation{
