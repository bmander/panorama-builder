...
func TestBridgeIterationCallback(t *testing.T) {
	...
	// RMS Ceres reports back through wire.Result. (Ceres' final state may differ
...
func TestBridgePlumbConstraintBitEqualSharedAxes(t *testing.T) {
	...
	prob.CPConstraints = append(prob.CPConstraints, wire.CPConstraint{
	...
	// Find the two CP changes in wire.Result.Changes and check their post-solve
	...
	var aChange, bChange *wire.EntityChange
...
// TestBridgeSingleStationFocus exercises the wire.ModeSingleStation scoping path
...
func TestBridgeSingleStationFocus(t *testing.T) {
	...
	res, err := core.Solve(prob, core.Config{Mode: wire.ModeSingleStation, FocusID: "s1"})
	if err != nil {
		t.Fatalf("solve: %v", err)
	}
	...
	if sawP2 {
		t.Errorf("unexpected p2 (non-focus) change p2Az=%v; should be locked under wire.ModeSingleStation", p2Az)
	}
...
// with fewer than two fully-locked stations must return wire.ErrUnderconstrainedGauge.
...
func TestBridgeUnderconstrainedGauge(t *testing.T) {
	prob := wire.Problem{
		...
		ControlPoints: []wire.ControlPoint{
	...
	if !errors.Is(err, wire.ErrUnderconstrainedGauge) {
		t.Errorf("expected wire.ErrUnderconstrainedGauge; got %v", err)
	}
}
