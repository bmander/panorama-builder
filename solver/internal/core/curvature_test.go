...
func TestCurvatureBiasGoneOnLongBaseline(t *testing.T) {
	...
	dLng := halfBaselineM / (geom.MPerDegLat * cosLat)
	stALng := baseLng - dLng
	stBLng := baseLng + dLng
	cpLng := baseLng
	...
	// unobservable).
	cpLat := baseLat + 200.0/geom.MPerDegLat
	...
	res, err := core.Solve(prob, core.Config{
		Mode: wire.ModeSingleControlPoint, FocusID: "cp1", MaxIters: 80,
	})
	if err != nil {
...
func photoAzToward(sLat, sLng, tLat, tLng float64) float64 {
	az, _ := geom.BearingFromStationToCP(sLat, sLng, 0, tLat, tLng, 0)
	return az
}
