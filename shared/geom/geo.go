// Package geom provides pure-Go geodesy and projection math used by both
// binaries. It depends only on the standard library and shared/wire for
// the few data types the math operates on (Pose, etc.).
package geom
...
// the polar singularity bounded. Used by both the solver's Ceres residual
// (in core) and the api's per-CP fit computation (control_point_fits.go).
