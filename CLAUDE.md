...
- **New entity type?** Implement the `entitySpec` interface (`entities.go`) and add the new spec to `entityRegistry`. The interface forces all per-entity behavior — `rank`, `feedsDateGraph`, `table`, `cols`, `scanJSON`, `insert`, `update` — to exist (a missing method won't compile), and `entityRank` / `insertEntityFromJSON` / `updateEntityFromJSON` / `deleteEntityByID` / `loadEntityJSONsByType` / `isDateGraphEntity` all derive from the registry. The two things the compiler can't see — that you added the `entityX` const to `entityRegistry`, and that the `session_ops.entity_type` CHECK constraint (migration, see `0022_session_ops_cp_surface.sql`) lists it — are guarded by the parity tests in `entities_test.go`. So the remaining manual steps are: add the spec to the registry literal and add a migration extending the CHECK constraint.
